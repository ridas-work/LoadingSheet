...
1. **PO team** creates orders at `/new-order`. The page lists **every catalog product**; type **carton counts** for a normal order, or choose **Mixed sample box** to ship several products in **one physical carton** (enter **bottles per product** + how many identical mixed boxes). On a **standard** PO you can also use **Add custom carton** to define extra physical boxes that each contain **several products with bottle counts** (same PO as normal carton lines). Leave unused catalog rows blank. Toggle **Sample / custom** on standard orders for non-default bottles per carton. Include **city** and **deadline** for the management report. Design notes: `.planning/phases/22-hybrid-order-custom-boxes/`.
...
3. **Rashid** lands on **`/dispatch/trips`**: create a **vehicle trip** with one or more POs, enter vehicle/driver/footer once (synced to every linked sheet). Per PO, use **Assign batches** on the trip page or **Edit dispatch** on the loading sheet for batch rows only when the order is on a trip. **`/dispatch/filling`** — daily filling log: choose one or more product packings per batch and enter **bottles filled today** plus **bottles ready to deliver** (fully capped, labeled/stickered, packed/finished), while **Physical remaining** stays in liters for liquid reconciliation. **Variance / waste (L)** uses derived liters from bottle counts against Nimra’s system remaining.
4. **Haider** lands on **`/dispatch/inventory`** and maintains packaging stock (empty bottles, caps, stickers/labels, cartons/boxes, pouches, partitions). Product catalog `bottlesPerCarton` controls carton use, e.g. Rhino 250ml has 20 bottles/carton.
5. **Zaman** at **`/gate/orders`**: only orders that are **ready to go** appear — on a **dispatch trip** with **vehicle, driver, and DC** filled by Rashid. Zaman marks **Out for delivery** when the vehicle leaves the gate; **Delivered** when the customer has received the goods, which automatically deducts mapped packaging stock; **Pending redelivery** if goods return on the vehicle for a later run.
6. **Anyone** can **View loading sheet** and print.
7. **Waleed Intisar** signs in at `/admin` for the **pending orders** grid. He can browse **Orders** and use **Edit order** to correct PO details or product quantities when a customer changes an order or material has an issue — only the boss can edit; PO team creates orders but cannot change them after submit. Other oversight (**Production batches**, **Dispatch trips**, **Packaging inventory**, **Daily filling**) is read-only.
