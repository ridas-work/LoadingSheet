...
1. **PO team** creates orders at `/new-order`. The page lists **every catalog product**; just type the **carton count** next to the products on this order — leave the rest blank. Toggle **Sample / custom** if a product ships with non-standard bottles per carton (e.g. 1-bottle samples). Add an **Other** row only for products not in the catalog. Include **city** and **deadline** for the management report.
