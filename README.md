...
4. **Zaman** at **`/gate/orders`**: after Rashid puts an order on a trip (or records vehicle/DC on dispatch), Zaman marks **Out for delivery** when the vehicle leaves the gate; **Delivered** when the customer has received the goods; **Pending redelivery** if goods return on the vehicle for a later run.
